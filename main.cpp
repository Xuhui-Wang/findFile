...
#include <vector>
#include <string.h>
// This is a program to find k most frequent words in a file
#include <ctype.h>
...
int main()
{
	DIR *dir;
	struct dirent *ent;
	vector<string> files_neg, files_pos;
	if ((dir = opendir ("E:\\Java\\homework1\\P2_Logistic\\train\\neg")) != NULL) {
	  /* print all the files and directories within directory */
	  while ((ent = readdir (dir)) != NULL) {
		//printf ("%s\n", ent->d_name);
		files_neg.push_back(ent->d_name);
	  }
	  closedir (dir);
	} else {
	...
	files_neg.erase(files_neg.begin(), files_neg.begin() + 2);
	if ((dir = opendir ("E:\\Java\\homework1\\P2_Logistic\\train\\pos")) != NULL) {
	  /* print all the files and directories within directory */
	  while ((ent = readdir (dir)) != NULL) {
		//printf ("%s\n", ent->d_name);
		files_pos.push_back(ent->d_name);
	  }
	  closedir (dir);
	} else {
	  /* could not open directory */
	...
	files_pos.erase(files_neg.begin(), files_neg.begin() + 2);
	cout << files_neg[0]<< endl;
    ...
    for (int i = 0; i < files_neg.size(); ++i) {
        string s = files_neg[i];
        s = "E:\\Java\\homework1\\P2_Logistic\\train\\neg\\" + s;
...
//        string s = files_pos[i];
//        FILE *fp = fopen("E:\\Java\\homework1\\P2_Logistic\\train\\pos\\" + s, "r");
//        while (fscanf(fp, "%s", buffer) != EOF)
//            insertTrieAndHeap(buffer, &root, minHeap);
//    }
